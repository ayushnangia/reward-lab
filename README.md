...
**Export video** records the current experiment from update zero, using the algorithms on the current chart page and the main **Speed** setting. Choose a clip limit of 15, 30, or 60 seconds, or **Full run**. A limited clip ends after the last complete update that fits; it never squeezes the full run into the limit or changes the note lengths. The dialog shows the included updates and actual duration. Sampling, weighting, updating, and sound scans use the live playback timing. The selected algorithm's sound uses the site's `audio.js` synthesis and current volume, with no narration or background music. The 1920×1080 canvas targets 60 fps; actual frame rate depends on the browser and device. Export sound can be turned off independently of live playback. Export uses a separate simulation with the same seed. Keep the tab visible while recording; Cancel or Escape stops the export. Preview the completed clip before downloading. Browsers with native MP4 recording use MP4; others use WebM and show the format before recording. Everything runs locally without uploads, recording permissions, or added runtime dependencies.
