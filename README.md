...
**Export video** saves a replay of the current experiment, from update zero to the chosen horizon, with the algorithms on the current chart page. Choose 15, 30, or 60 seconds. The 1920×1080 canvas targets 60 fps; actual frame rate depends on the browser and device. The default soundtrack uses the selected algorithm and current volume through the same `audio.js` synthesis as the site. Each policy transition finishes before its sound scan, and both follow the clip's duration. There is no narration or background music. Export sound can be turned off independently of live playback. The export uses a separate simulation with the same seed, preserving the on-screen run. Keep the tab visible while it records; Cancel or Escape stops the export. Preview the completed clip before downloading. Browsers with native MP4 recording use MP4; others use WebM and show that format before recording. Export runs entirely on your device without uploads, a recording permission prompt, or added runtime dependencies.
